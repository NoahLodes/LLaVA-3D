...
class Preprocessor():
    def __init__(self, skip_existing=False, distance="avg"):
        ...
        self.word2idx = {}
        index = 0
        file = open(os.path.join(CONF_PATH_R3SCAN_RAW, "classes.txt"), 'r')
        ...
        self.rel2idx = {}
        index = 0
        file = open(os.path.join(CONF_PATH_R3SCAN_RAW, "relationships.txt"), 'r')
        category = file.readline()[:-1]
        while category:
        ...
        self.boxes_train = json.load(open(CONF_PATH_R3SCAN_RAW+'/obj_boxes_train_refined.json', 'r'))
        self.boxes_val = json.load(open(CONF_PATH_R3SCAN_RAW+'/obj_boxes_val_refined.json', 'r'))
    ...
    def process_one_scan(self, relationships_scan, scan_id):
        ...
        scan = scan_id[:-2]
        pcl_array, rgb_array, normals_array = load_mesh(os.path.join(CONF_PATH_R3SCAN_RAW, "{}/mesh.refined.v2.obj".format(scan)),
                                                        os.path.join(CONF_PATH_R3SCAN_RAW, "{}/mesh.refined_0.png".format(scan)))

        object2fame = pickle.load(open(os.path.join(CONF_PATH_R3SCAN_PROCESSED, 'views', f"{relationships_scan['scan']}_object2image.pkl"), "rb"))

        segments = {}  # key:segment id, value: points belong to this segment
        with open(os.path.join(CONF_PATH_R3SCAN_RAW, "{}/mesh.refined.0.010000.segs.v2.json".format(scan)), 'r') as f:
        ...
        with open(os.path.join(CONF_PATH_R3SCAN_RAW, "{}/semseg.v2.json".format(scan)), 'r') as f:
...
if __name__ == '__main__':
    ...
    relationships_train = json.load(open(os.path.join(CONF_PATH_R3SCAN_RAW, "relationships_train.json")))["scans"]
    relationships_val = json.load(open(os.path.join(CONF_PATH_R3SCAN_RAW, "relationships_validation.json")))["scans"]
